...
public class StatAllocationModule  {
	...
	public void DisplayStatAllocationModule() {
		...
		if(availPoints == 0){
		if(GUI.Button(new Rect(Screen.width/2-100,Screen.height/4*3,100,50), "Finish Stats")){
	...
	private void Reset(){
		...
		GameInformation.Strength= playerClass.Strength;
		GameInformation.Vitality=playerClass.Vitality;
		GameInformation.Dexterity=playerClass.Dexterity;
		GameInformation.Intellect=playerClass.Intellect;
		pointsToAllocate = new int[10];
		baseStatPoints = new int[10];
		availPoints = 10;
	}
		
		public void RetrieveBaseStatPoints(){
		BaseCharacterClass playerClass = GameInformation.PlayerClass;
